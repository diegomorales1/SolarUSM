...
if ($_POST){
    ...
    $sql = "SELECT clave_transportista FROM transportista WHERE nombre_transportista = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $hashContrasena = $row["clave_transportista"];
        // Verificar si la contraseña coincide con el hash almacenado
        if ($contrasena === $hashContrasena){
            $_SESSION["loggedin"] = true;

            //Esto extrae info usuario
            $query = "SELECT * FROM transportista WHERE nombre_transportista = '$nombreUsuario'";
            $result = $conn->query($query);
            $row2 = $result->fetch_assoc();
            $id_usuario = $row2['id_transportista'];
            $nombre_usuario_actual = $row2['nombre_transportista'];
            //SESSION que guarda toda la info del usuario
            $_SESSION["idusuario"] = $row2;
            $_SESSION["nombre_u"] = $nombre_usuario_actual;

            header("location:trabajos_transportistas.php");
        } else {
